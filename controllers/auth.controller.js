...
export const signUp = async (req, res, next) => {
  ...
  try {
    ...
    const existingUser = await User.findOne({ email }) // Change 'user' to 'User'
    ...
    const token = jwt.sign(
      { userId: newUser[0]._id }, // Fix: remove JWT_SECRET from here
      JWT_SECRET,
    ...
    res.status(201).json({  // Fix: move closing brace
      success: true,
